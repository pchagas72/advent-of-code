...
/*func return_new_common_digits(common_digits []string) ([]string){
	...
	for i := 0; i < len(common_digits); i++{
		...
		var element string = common_digits[i];

		for k := 0; k < len(common_digits); k++{

			var digit string = string(element[k]);
...
func return_common_starting_numbers(common_digits []string, overall_digits []string) ([]string){

	//var final_digit string = "";
	var least_common_digits []string = []string{};
	...
	for i := 0; i < len(common_digits); i++{
		...
		common_digits, least_common_digits = return_most_common_digits(new_digit_list);
...
func main(){
	...
	//def_array, err := read_file("../data/3st.txt");
	//check_error(err);
	check_error(err);
	
	var digit_list [][]string = return_digit_list(test_array);

	fmt.Println(return_most_common_digits(digit_list));
	most_common_digits, least_common_digits := return_most_common_digits(digit_list);
	fmt.Println(least_common_digits);

	var oxygen []string = return_common_starting_numbers(most_common_digits, test_array);
